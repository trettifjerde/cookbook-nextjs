...
const container = {
    base: 'gap-2 p-2 border rounded-md hover:text-green transition-colors duration-200',
    ...
    notActive: 'border-gray-300 bg-white',
    active: 'text-green border-green bg-green-shadow',
    currentClass(isActive: boolean, hasImage: string) {
        return `${this.base} ${this.height} ${hasImage ? this.withImage : this.withoutImage} ${isActive ? this.active : this.notActive}`;
    },
};
